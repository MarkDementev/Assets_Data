...
public abstract class AssetRelationship {
    ...
    /**
     * Поле ниже представляет собой мапу с указанием того, каким владельцам принадлежит какое количество активов
     * в рамках пакета, который обслуживает данная сущность. Т.к. эта сущность может быть использована не только для
     * финансовых активов, вэльюс мапы имеет тип float. Облигации, к примеру, должны принадлежать полностью одному
     * собственнику, и в пакете их может быть тысячи, а вот квартира будет считаться как один актив в пакете, но доли
     * владения ей уже исчисляются процентами, для чего нужно использовать не int, как с облигациями, а float.
     */
    @ElementCollection
